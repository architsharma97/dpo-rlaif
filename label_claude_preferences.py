...
def main(base_dir, model1_name, model2_name, max_length, max_num_comparisons):
    def _get_model_outputs_and_instructions(name):
        if name == 'chatgpt':
            return get_chatgpt_outputs()
        else:
            return process_llama_samples_from_dir(os.path.join(base_dir, f'sharegpt2turn_noeos_maxlen{max_length}_{name}'))
    
    matched_instruction_outputs = match_instruction_outputs(*_get_model_outputs_and_instructions(model1_name), *_get_model_outputs_and_instructions(model2_name))
...
if __name__ == '__main__':
    ...
    main(base_dir=args.base_dir, model1_name=args.model1_name, model2_name=args.model2_name, max_length=args.max_length, max_num_comparisons=args.max_num_comparisons)
